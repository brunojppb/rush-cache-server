...
serverProcess.on("error", (err) => {
  console.error(`Failed to start Rush Cache Server: ${err.message}`);
  process.exit(1);
});
...
if (!isHealthy) {
  console.error("\nFailed to start Rush Cache Server within 30 seconds.");
...
console.log(`
${colorString(consoleColor.FgGreen, "Rush Cache Server running with pid:")} "${pid}"
...
saveState(SERVER_PID_KEY, pid?.toString());
process.exit(0);
